...
const BG_OPTIONS = [
  { label: "Red (Primary)", value: "bg-(--color-danger)" },
  { label: "Orange (Accent)", value: "bg-(--color-accent)" },
  { label: "Dark Green", value: "bg-(--color-primary)" },
  { label: "Dark Green Gradient", value: "bg-linear-to-b from-(--color-primary) to-(--color-primary-dark)" },
  { label: "Light Orange", value: "bg-(--color-primary-light)" },
  { label: "Light Accent", value: "bg-(--color-accent-light)" },
];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input value={form.href} onChange={(e) => set("href", e.target.value)} required className="w-full text-sm px-3 py-2 border border-gray-200 rounded-lg" placeholder="/shop" />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">Left Background</label>
          <select value={form.leftBg} onChange={(e) => set("leftBg", e.target.value)} className="w-full text-xs px-2 py-2 border border-gray-200 rounded-lg bg-white">
            {BG_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            <option value={form.leftBg}>{form.leftBg.startsWith("bg-(") || BG_OPTIONS.find(o => o.value === form.leftBg) ? undefined : "Custom"}</option>
          </select>
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">Right Background</label>
          <select value={form.rightBg} onChange={(e) => set("rightBg", e.target.value)} className="w-full text-xs px-2 py-2 border border-gray-200 rounded-lg bg-white">
            {BG_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>
      </div>
