  ...
  return (
    <footer className="bg-(--color-footer-bg) text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {/* Brand */}
        <div>
          <Image src="/images/logo-1.png" alt="Dollar Shop" width={100} height={86} className="h-24 w-auto object-contain mb-4" />
          <p className="text-sm text-white/70 leading-relaxed mb-4">
            Your neighbourhood store for everyday essentials. Quality Everyday. Every Dollar Counts.
          </p>
          <div className="flex gap-3">
            <a href="https://wa.me/263772566468" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="w-9 h-9 bg-white/10 hover:bg-(--color-primary) rounded-full flex items-center justify-center transition-colors duration-150">
              <MessageCircle size={16} />
            </a>
            <a href="https://facebook.com/dollarshopzw" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-9 h-9 bg-white/10 hover:bg-(--color-primary) rounded-full flex items-center justify-center transition-colors duration-150">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>
            </a>
            <a href="https://instagram.com/dollarshopzw" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-9 h-9 bg-white/10 hover:bg-(--color-primary) rounded-full flex items-center justify-center transition-colors duration-150">
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-white/70">
            ...
            <li className="flex gap-2.5">
              <Clock size={15} className="shrink-0 mt-0.5 text-(--color-primary)" />
